...
def gen_tuple(model_img,model_name,best_param,instance):
    ...
    beta = param
    siblings = [instance*2+i for i in range(2)]
...
if __name__ == "__main__":
    ...
    for model_path in model_path_list:
        ...
        for model_img_type_list in model_img_split_list:
            ...
            if rv <= 0.8:
                model_img = model_img_type_path+"{}_{}_{}".format(best_param[0],best_param[1],best_param[2])+".png"
                data.append(gen_tuple(model_img,model_name,best_param,instance))
                
                for param in random.sample(model_non_ideal_list,int(sys.argv[3])-1):
                    model_img = model_img_type_path+"{}_{}_{}".format(param[0],param[1],param[2])+".png"
                    data.append(gen_tuple(model_img,model_name,best_param,instance))
            else:
                for param in random.sample(model_non_ideal_list,int(sys.argv[3])):
                    model_img = model_img_type_path+"{}_{}_{}".format(param[0],param[1],param[2])+".png"
                    data.append(gen_tuple(model_img,model_name,best_param,instance))
                    ##    
